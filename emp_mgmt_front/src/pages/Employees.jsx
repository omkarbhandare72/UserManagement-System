          ...
          <div className="col-md-4">
            <div className="card shadow border-0">
              <div className="card-body p-4">
                <h3 className="mb-4">{editingId ? "Update Employee" : "Add Employee"}</h3>
                <form onSubmit={editingId ? updateEmployee : addEmployee}>
                  ...
                  <div className="mb-3">
                    <input type="number" className="form-control" placeholder="Salary" name="salary" value={formData.salary} onChange={handleChange} required />
                  </div>
                  <button className="btn btn-primary w-100">{editingId ? "Update Employee" : "Add Employee"}</button>
                  {editingId && (
                    <button type="button" className="btn btn-secondary w-100 mt-2" onClick={cancelEdit}>Cancel</button>
                  )}
                </form>
              </div>
            </div>
                  ...
                  <tbody>
                    {employees.map((employee) => (
                      <tr key={employee.id}>
                        ...
                        <td>
                          <button className="btn btn-warning btn-sm me-2" onClick={() => editEmployee(employee)}>Edit</button>
